...
void AMonsterShooterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Fire", IE_Released, this, &AMonsterShooterCharacter::OnFire);
...
void AMonsterShooterCharacter::DealDamage(float DamageAmount)
{
	...
	if (Health <= 0.0f)
	{
		AMonsterShooterGameMode* myGameMod = Cast<AMonsterShooterGameMode>(UGameplayStatics::GetGameMode(GetWorld()));

		if (myGameMod)
		{
			myGameMod->RestartGameplay(false);
		}
		Destroy();
	}
}
